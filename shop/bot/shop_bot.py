...
@app.route(WEBHOOK_URI, method=['POST'])
def handle_webhook():
    if request.headers.get('content-type') == 'application/json':
        ...
        return ''
    abort(403)  # доступ запроса к боту не разрешон
...
def handler_news(message: Message):
    ...
    """
    news = sorted(News.objects(), key=lambda obj: obj.created, reverse=True)
    for i in range(5):
        bot.send_message(
            message.chat.id,
...
def handler_news(message: Message):
    ...
    """
    products = Product.objects(discount__gt=0)
